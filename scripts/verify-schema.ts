...
/**
 * Schema verification report for migrations 001–029 (Phase 1 + Phase 2).
...
// Load .env.local / .env files before reading process.env
...
async function main() {
  ...
  console.log(
    `\nStorage bucket 'documents' private: ${
      report.bucketPrivate === true
        ? "yes"
        : report.bucketPrivate === false
        ? "NO — CRITICAL"
        : "unknown"
    }`
  );
