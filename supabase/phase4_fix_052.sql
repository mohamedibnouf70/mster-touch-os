-- Master Touch OS ? phase4_fix_052
-- Apply this file in the Supabase Dashboard SQL Editor.
-- Equivalent to migrations/052_phase4_employee_master_hardening.sql
-- Do NOT re-run 001?051. Do NOT modify 001?051.
...
values
  ('employee.create', 'employee', 'create', '????? ???? / ???? ???? ?????', 'Create employee / limited login account'),
  ('employee.read_sensitive', 'employee', 'read_sensitive', '????? ???? ???? ????? (??? ??????)', 'Read sensitive non-compensation employee fields'),
  ('employee_compliance.read', 'employee_compliance', 'read', '????? ?????? ???????? ????????', 'Read employee compliance records'),
  ('employee_compliance.manage', 'employee_compliance', 'manage', '????? ?????? ???????? ????????', 'Manage employee compliance records'),
  ('hr_alert.read', 'hr_alert', 'read', '????? ??????? ??????? ???????', 'Read HR alert hooks'),
  ('hr_alert.manage', 'hr_alert', 'manage', '????? ??????? ??????? ???????', 'Manage HR alert hooks')
