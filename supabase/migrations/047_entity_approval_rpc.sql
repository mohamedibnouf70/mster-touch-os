-- Master Touch OS — 047
-- Atomic entity approval decision to keep approval engine and commercial state synchronized.
...
begin
  ...
  select public.submit_approval_decision(v_step.id, p_official_code, p_comment) into v_action;
  ...
  if v_request.status = 'completed' then
    if v_request.entity_type = 'purchase_request' then
      ...
      if v_request.official_outcome in ('approved', 'approved_as_noted') then
        ...
        from public.approve_variation(v_variation.id, coalesce(v_variation.submitted_amount, v_variation.cost_impact, 0));
      elsif v_request.official_outcome = 'rejected' then
